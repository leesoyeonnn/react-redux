...
module.exports = {
  ...
  output: {
    ...
    // !! 디렉토리 명을 지정할 때 OS 별로 디렉토리 파일 시스템의 구조가 조금 달라서 개발자 컴퓨터 환경에서만 온전하게 동작한다면 문자열로 지정해줘도 되지만 
  ...
  module: {
    rules: [ 
      // rules 안에 지정하는 것을 loader라고 한다. npm으로 설치했던 babel loader를 지정해주면 된다.
      // rules 키워드 : n개의 loader를 지정할 수 있으므로 배열로 정의
      // use 키워드 : 각각의 loader가 어떻게 동작할지에 대해서는 객체로 정의
      {
          // babel loader가 처리해야 할 파일은 javaScript 파일인데 입력되는 모든 파일이 js이외에 
          // 텍스트, 이미지, json, css 등등 처리해줄 수 없는 파일이 webpack을 통해서 들어올 수도 있다.
          // js파일이지만 transpile될 필요 없는 파일도 제거해줘야 한다. (e.g. node_modules 내의 js 파일) 포함되면 bundling됐을 때 크기가 너무 커질 수 있음
          // 처리해줄 수 없는 파일은 미리 제거해줄 수 있다. -> test, exclude 속성 사용
